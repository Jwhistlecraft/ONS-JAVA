		...
		Create.addActionListener(
				...
				new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						String AccNoGenerator="select concat('"+AccNo+"',lpad(coalesce(max(substr(AccNo,3,3))+1,'001'),3,'0'))  as newacno from accounts where substring(accno,1,1)='"+AccNo.substring(0,1)+"'";
						
						ResultSet AccNoJavaSave=ServerQ.executeQuery(AccNoGenerator);
							AccNoJavaSave.next();
							String NewAccountNumber=AccNoJavaSave.getString("newacno");
							int StartingBalance = 0;
						
						String InsertAccount="insert into Accounts values('" + NewAccountNumber + "','" + "" + Name.getText() + "'" + "," + "'" + Address.getText() + "','" + StartingBalance + "')";
						
						ServerQ.executeUpdate(InsertAccount);
